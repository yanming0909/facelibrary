...
public class AutoTexturePreviewView extends FrameLayout {
    ...
    private int textureHeigh = 0;

    private int previewWidth = 0;
    private int previewHeight = 0;

    public AutoTexturePreviewView(@NonNull Context context) {
        super(context);
        init();
    }
    ...
    public AutoTexturePreviewView(@NonNull Context context, @Nullable AttributeSet attrs,
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (previewWidth * videoHeight > previewHeight * videoWidth) {
            ...
            textureView.layout(0, (previewHeight - scaledChildHeight) / 2,
                    previewWidth, (previewHeight + scaledChildHeight) / 2);
        } else {
            int scaledChildWidth = videoWidth * previewHeight / videoHeight;
            textureView.layout((previewWidth - scaledChildWidth) / 2, 0,
                    (previewWidth + scaledChildWidth) / 2, previewHeight);

        }

        textureWidth = textureView.getWidth();
        textureHeigh = textureView.getHeight();
    }
